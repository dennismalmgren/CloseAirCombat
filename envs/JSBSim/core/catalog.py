...
from ..utils.utils import in_range_rad
...
class ExtraCatalog(Property, Enum):
    ...
    missions_cruise_target_position_h_sl_m = Property(
        "missions/cruise-target-position-h-sl-m",
        "target altitude MSL [m]",
        JsbsimCatalog.position_h_sl_ft.min * 0.3048,
        JsbsimCatalog.position_h_sl_ft.max * 0.3048,
    )

    missions_cruise_target_attitude_heading_true_rad = Property(
        "missions/cruise-target-attitude-heading-true-rad",
        "target true heading [rad]",
        JsbsimCatalog.attitude_heading_true_rad.min,
        JsbsimCatalog.attitude_heading_true_rad.max,
    )

    missions_cruise_target_velocities_vc_mps = Property(
