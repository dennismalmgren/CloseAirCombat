...
class OpusSmoothingReward(BaseRewardFunction):
    ...
    #    self.reward_item_names = [self.__class__.__name__ + item for item in ['', '_heading', '_alt', '_roll', '_speed', '_smoothness']]
        self.reward_item_names = [self.__class__.__name__ + item for item in ['', '_heading', '_alt', '_roll', '_speed', '_smoothness_p', '_smoothness_w', '_smoothness_p_ref', 'smoothness_w_ref']]
    ...
    def get_reward(self, task, env, agent_id):
       ...
       # smoothness_r = 0.000001 * np.sum(smoothness_variables**2)
        ...
        smoothness_value_p = np.sum(np.abs(np.diff(smoothness_p, n=1)))
 ...
 #* smooth_roll_r * smooth_w_r
        ...
        if (heading_r * alt_r * roll_r * speed_r) >= 1.0:
            reward += (smooth_roll_r * smooth_w_r)**(1 / 2)
        else:
            smooth_roll_r = smooth_w_r = 0.0

       # return self._process(reward, agent_id, (heading_r, alt_r, roll_r, speed_r, smoothness_r))
        return self._process(reward, agent_id, (heading_r, alt_r, roll_r, speed_r, smooth_roll_r, smooth_w_r, smooth_p_ref, smooth_w_ref))
