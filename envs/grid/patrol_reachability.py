...
def manhattan(start_point, end_grid_h, end_grid_w):
    return abs(start_point[0] - end_grid_h) + abs(start_point[1] - end_grid_w)
...
def fill_in_dir(distances, index_grid_h, index_grid_w, height, width, start_dir):
    for start_h in range(height):
        for start_w in range(width):
            ...
            start_point = np.asarray([start_h, start_w])
            ...
            end_w = slice(0, width)
            distances[start_h, start_w, start_dir, end_h, end_w, end_dir] = np.inf
...
def calc_distance_matrix(start_height, start_width):
    dirs = 4
    distances = np.zeros((start_height, start_width, dirs, start_height, start_width, dirs))
    ...
    for start_dir in range(4):
        if start_dir == 0:
            ...
            height = start_width
            width = start_height
            distances = np.rot90(distances, k = 1, axes=(0, 1))
            distances = np.rot90(distances, k = 1, axes=(3, 4))
        elif start_dir == 2:
            height = start_height
            width = start_width
            ...
            height = start_width
            width = start_height
            distances = np.rot90(distances, k = 3, axes=(0, 1))
            distances = np.rot90(distances, k = 3, axes=(3, 4))
        
        heights = np.arange(height)
        widths = np.arange(width)
        index_grid = np.meshgrid(heights, widths, indexing='ij')
        index_grid_h = index_grid[0]
        index_grid_w = index_grid[1]
        fill_in_dir(distances, index_grid_h, index_grid_w, height, width, start_dir)

        if start_dir == 1:
            distances = np.rot90(distances, k = 1, axes=(1, 0))
            distances = np.rot90(distances, k = 1, axes=(4, 3))
        elif start_dir == 2:
            distances = np.flip(distances, axis=0)
            distances = np.flip(distances, axis=3)
        elif start_dir == 3:
