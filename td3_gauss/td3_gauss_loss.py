...
class TD3GaussLoss(LossModule):
    ...
    def actor_loss(self, tensordict):
        ...
        state_action_pmf = state_action_value_actor.exp()
        state_action_value_actor = (state_action_pmf * self.support).sum(-1, keepdim=True)
    ...
    def value_loss(self, tensordict):
        ...
        with torch.no_grad():
            ...
            state_action_value_pmf = next_target_q1q2.exp()
            next_target_q1q2 = (state_action_value_pmf * self.support).sum(-1)
        ...
        loss_qval = torch.nn.functional.cross_entropy(current_qvalue, target_value, reduction="none").sum(0)
