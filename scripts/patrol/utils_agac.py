...
def make_ppo_modules(proof_environment):
    ...
    out_keys_common_adv = ["common_features_adv"]
    ...
    common_module_adv = TensorDictModule(
        module=copy.deepcopy(input_module),
        in_keys=in_keys_common,
        out_keys=out_keys_common_adv,
    )
    ...
    policy_module_adv = TensorDictModule(
        module=copy.deepcopy(policy_net),
        in_keys=["common_features_adv"],
        out_keys=["logits_adv"],
    )
    ...
    # Add probabilistic sampling of the actions
    policy_module_adv = ProbabilisticActor(
        policy_module_adv,
        in_keys={
                    "logits": "sample_logits_adv", 
                    "mask": "mask"
        },
        spec=CompositeSpec(action=proof_environment.action_spec),
        distribution_class=distribution_class,
    ...
    return common_module, policy_module, common_module_adv, policy_module_adv, value_module

def make_ppo_models(device):
    proof_environment = make_parallel_env(1, device=device)

    common_module, policy_module, common_module_adv, policy_module_adv, value_module = make_ppo_modules(
        proof_environment
    )
    ...
    common_module_adv = common_module_adv.to(device)
    policy_module_adv = policy_module_adv.to(device)
    actor_critic = actor_critic.to(device)

    with torch.no_grad():
        td = proof_environment.rollout(max_steps=100, break_when_any_done=False)
        td = actor_critic(td)
        td = common_module_adv(td)
        td = policy_module_adv(td)
        del td
    ...
    return actor, critic, common_module_adv, policy_module_adv
