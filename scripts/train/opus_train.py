    ...
    for i, tensordict in enumerate(collector):
        ...
        if collected_frames >= init_random_frames:
            ...
            for i in range(num_updates):
                ...
                actor_loss.backward()
                torch.nn.utils.clip_grad_norm_(actor_params, cfg.optim.max_grad_norm)
                optimizer_actor.step()

                # Update critic
                optimizer_critic.zero_grad()
                q_loss.backward()
                torch.nn.utils.clip_grad_norm_(critic_params, cfg.optim.max_grad_norm)
                optimizer_critic.step()

                # Update alpha
                optimizer_alpha.zero_grad()
                alpha_loss.backward()
                torch.nn.utils.clip_grad_norm_(alpha_params, cfg.optim.max_grad_norm)
