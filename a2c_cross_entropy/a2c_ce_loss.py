...
class A2CCELoss(LossModule):
    ...
    def _construct_gauss_target(
        ...
        action_batch = action_batch.clamp(self.support.min(), self.support.max())
        ...
        m = m.reshape(*action.shape[:-1], action.shape[-1], self.support.numel())
    ...
    def _gauss_log_probs(
        ...
        action = tensordict.get(self.tensor_keys.action)
        action_reshaped = action.unsqueeze(-1).expand((-1, -1, len(self.support))).reshape((-1, 1))
        with self.actor_network_params.to_module(
            self.actor_network
        ) if self.functional else contextlib.nullcontext():
            td = self.actor_network(tensordict.clone())

        action_mean_logits = td.get("loc_logits")
        action_mean_logits = action_mean_logits.reshape(-1)
        action_scale = td.get("scale") \
                                .unsqueeze(-1) \
                                .expand((-1, -1, len(self.support)))
        action_support = self.support.unsqueeze(0).unsqueeze(0).expand_as(action_scale)
        action_dists = TanhNormal(
            loc = action_support.reshape((-1, 1)),
            scale = action_scale.reshape((-1, 1))
            )
        action_reshaped_logprobs = action_dists.log_prob(action_reshaped)
        action_logprobs = action_reshaped_logprobs + action_mean_logits
        action_logprobs = action_logprobs.reshape(*action.shape, len(self.support))
    ...
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        action_targets = self._construct_gauss_target(tensordict)
        action_target_shape = action_targets.shape

        action_targets = action_targets.reshape((-1, len(self.support)))
        gauss_log_probs = self._gauss_log_probs(tensordict).reshape((-1, len(self.support)))
